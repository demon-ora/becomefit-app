...
class MyAdapter1 extends RecyclerView.Adapter<MyAdapter1.MyViewHolder> {
    ...
    int h = 1;
    public MyAdapter1(Context context,ArrayList<data> mlist){
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        if(mlist.get(7).getType().equals("full")){
            data uploadcurrent = mlist.get(7);
            holder.textView.setText(uploadcurrent.getName());
            data uploadcurrent1 = mlist.get(7);
            holder.des.setText(uploadcurrent1.getDes());
            Glide.with(context).load(mlist.get(7).getImageurl()).into(holder.imageView);}
        holder.button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                while (h < mlist.size()) {
                    if (mlist.get(h).getType().equals("full")) {
