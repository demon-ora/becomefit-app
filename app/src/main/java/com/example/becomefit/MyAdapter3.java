...
class MyAdapter3 extends RecyclerView.Adapter<MyAdapter3.MyViewHolder> {
    ...
    int h = 1;
    public MyAdapter3(Context context,ArrayList<data> mlist){
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        if(mlist.get(1).getType().equals("arm")){
            data uploadcurrent = mlist.get(1);
            holder.textView.setText(uploadcurrent.getName());
            data uploadcurrent1 = mlist.get(1);
            holder.des.setText(uploadcurrent1.getDes());
            Glide.with(context).load(mlist.get(1).getImageurl()).into(holder.imageView);}
        holder.button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                while (h < mlist.size()) {
                    if (mlist.get(h).getType().equals("arm")) {
